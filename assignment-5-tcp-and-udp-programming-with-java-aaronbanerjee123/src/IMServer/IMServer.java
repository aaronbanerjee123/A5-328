...
public class IMServer {
	...
	public void updateUserStatus(String userId, StatusRecord rec) { // Server would benefit from a timeout mechanism
	...
	public static boolean updateBuddyList(boolean isAdd, String userId, String buddyId) {
		try { // Read all names and make sure it is not already there, if not then add it
				// Simple algorithm: Read whole file to determine if buddy exists. Add at end if
				// doesn't.
	...
	public static boolean existingUser(String userId) { // Returns true only if user id exists on system
...
class IMRequest implements Runnable {
...

	{
		...
		try {
			...
			try {
				System.out.println(
						"Closing socket local:" + this.socket.getLocalPort() + " Remote:" + this.socket.getPort());
				this.socket.close();
			} catch (IOException e) {
	...
	private void sendResponse(String st) { // Sends one line response and closes socket and input/output buffers
		try {
			os.writeBytes(st);
		} catch (Exception e) {
			System.out.println(e);
	...
	private void processRequest() {
		...
		try {
			...
			if (!tokens.hasMoreTokens()) { // Invalid command/request as every command has at least one parameter
			...
			// Check command
			if (command.equals("REG")) { // Register user id
			...
			} else if (command.equals("ADD") || command.equals("DEL")) { // Check if valid existing user id
				if (!IMServer.existingUser(userId)) {
	...
	private boolean registerUser(String userId) { // Returns true if successfully register a user
		File userFile = new File(userId + ".txt");
		try {
		...
		try {
			...
			while (true) {
				...
				if (tokens.countTokens() >= 2) {
					...
					if (IMServer.existingUser(userId)) { // Must be valid user and command - ignore request otherwise
						if (command.equals("GET")) {
							...
							DatagramPacket sendPacket = new DatagramPacket(b.toString().getBytes(), b.length(),
									IPAddress, port);
							...
							if (tokens.countTokens() == 3) // Gives total of 5 tokens as already took out two
							{
								...
								if (statusMessage.equals(IMServer.onlineStatus)
										|| statusMessage.equals(IMServer.offlineStatus)
									...
									} catch (NumberFormatException e) { // Ignore exception but do not process invalid
																		// request
									}
